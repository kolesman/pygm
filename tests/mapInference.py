...
class testInference(unittest.TestCase):
    ...
    def setUp(self):
        ...
        f0 = pygm.Factor((0,), np.array([0.5, 0.5]), probability=True)
        f1 = pygm.Factor((1,), np.array([0.7, 0.3]), probability=True)
        f2 = pygm.Factor((2,), np.array([0.5, 0.5]), probability=True)
        f3 = pygm.Factor((3,), np.array([0.1, 0.9]), probability=True)
        f01 = pygm.Factor((0, 1), np.array([0.3, 0.2, 0.2, 0.3]).reshape(2, 2), probability=True)
        f12 = pygm.Factor((1, 2), np.array([0.3, 0.2, 0.2, 0.3]).reshape(2, 2), probability=True)
        f23 = pygm.Factor((2, 3), np.array([0.4, 0.1, 0.1, 0.4]).reshape(2, 2), probability=True)
        f30 = pygm.Factor((0, 3), np.array([0.4, 0.1, 0.1, 0.4]).reshape(2, 2), probability=True)
